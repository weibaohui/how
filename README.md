...
health cadence, so recovery no longer waits out the backoff.
...
| `livenesstimeout` | Wedge backstop (ms): an idle tunnel whose write queue is too full to be probed AND that has received no frame for this long is closed as dead. Normal dead-link detection is the active health-round probe (ping → pong within its own 10 s deadline), which replaced the old passive reaper — so any positive value is now used as-is (no ping-cadence floor anymore). Default 90000. 0 = default. |
