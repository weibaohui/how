...
| `livenesstimeout` | 卡死兜底（毫秒）：写队列被塞满无法探活、且这段时间内未收到任何帧的空闲隧道判定为死并关闭。同时是 Running 隧道回收阈值：收、发双方向都超过该时长无进展的 Running 隧道判定为半开卡死并关闭（阈值有 60s 下限——健康等待中的隧道只以 30s ping 节奏发送）。常规死链检测由主动健康巡检（探活 ping→pong，自带 10s 时限）承担，被动回收已被取代——因此任意正数配置均按原值生效（不再有 ping 周期下限）。默认 90000。0 = 默认。 |
