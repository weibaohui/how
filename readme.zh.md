...
- **服务端**（`livenesstimeout`，默认 120s）：一条隧道若在此时间内**未收到任何帧**（ping/pong/数据），即视为半开并关闭，确保下一个请求不会被派到死隧道上。死链约 2 分钟内被清理，而无需等操作系统的 TCP keepalive（约 2 小时）。其前还有两道更紧的防线：**派单新鲜度**（`dispatchfreshness`，默认 75s ≈ 客户端 30s ping 周期的 2.5 倍）——静默超过该时长的空闲隧道**不予派单、直接关闭换下一条**，客户端尚未察觉死链的隧道永远不会接到请求；**在忙保险丝**（`busylivenesstimeout`，默认 600s）——在忙隧道若超过该时长**未收到任何帧**即被关闭，这是双向分区场景（客户端的关闭帧同样到不了服务端）下挂起请求的最后兜底。注意：在健康链路上，客户端的 30s keepalive ping 在**任何**请求阶段（大文件上传、等上游响应头、SSE 事件间隔）都会持续刷新该水印，所以保险丝只会在 ping 彻底到不了时触发——链路死亡、客户端卡死、或路径完全积压（如调用方停止读取响应）——无需按请求静默期调大它。
- **客户端**（定时池健康巡检，`healthcheckinterval` 默认 30s，见下文）：客户端**主动探测**每条空闲隧道（发 ping、限时 10s 等 pong），关闭不应答的隧道并补拨——只有客户端能重建隧道，服务端无法回拨到客户端的内网。`livenesstimeout`（默认 90s）仅作为"卡死兜底"保留：写队列被塞满无法探活、且静默超过该时长的隧道判定为死。**若没有客户端侧检测，客户端会一直攥着一池死掉的半开隧道永不重连，服务端则会回收并清空 pool，第二天早上便报 "No proxy available"**——正是"白天好用、过一晚就死"的症状。

**定时池健康巡检**（`healthcheckinterval`，仅客户端，默认 30s）。它**就是**客户端的死链检测器：每隔一个间隔，主动探测每条空闲隧道（发 ping、限时 10s 等 pong），**打印每条隧道的状态**（`pool health: idle=3 ok=3 ... |tunnel#1:ok(0s) ...`），关闭不应答的隧道并立刻补拨，把池子恢复到 `poolidlesize` 条**经过验证**的隧道——服务端随时都有可用连接，无需任何人重启客户端。它取代了旧的按连接被动回收（被动回收要累计 90s 完全静默才能**察觉**死链、且无法端到端验证）；`livenesstimeout` 仅作为上文所述的卡死兜底保留。若池子已完全清空且 connector 正在拨号退避中（服务端挂过又回来了），每轮巡检仍会按巡检节奏补拨一条"救援"隧道——恢复不再需要等退避结束，也无需重启客户端。巡检还覆盖探活无法触及的两种状态：**Running** 隧道若收、发两个方向都超过 `livenesstimeout` 无任何进展（阈值有 60s 下限——健康隧道等上游首字节时仍能以 30s 节奏把 keepalive ping 发出去刷新"发"侧水印；双水印同时过期才说明 driver 在半开链路上卡死），即被关闭回收容量；**Connecting** 隧道超过预算（3 倍 `tunneltimeout` + 10s，未配置时 60s）未完成注册，即被关闭——对端"接受 TCP 后装死"再也不会永久泄漏池容量。
...
| `livenesstimeout` | 卡死兜底（毫秒）：写队列被塞满无法探活、且这段时间内未收到任何帧的空闲隧道判定为死并关闭。同时是 Running 隧道回收阈值：收、发双方向都超过该时长无进展的 Running 隧道判定为半开卡死并关闭（阈值有 60s 下限——健康等待中的隧道只以 30s ping 节奏发送）。常规死链检测由主动健康巡检（探活 ping→pong，自带 10s 时限）承担，被动回收已被取代——因此任意正数配置均按原值生效（不再有 ping 周期下限）。注意：流式响应刚结束时写队列可能尚未排空（暂时无法探活）而距上次 pong 已超过该值，低于 ~2×30s ping 周期（<60000ms）的配置可能在极窄窗口内误关健康隧道；默认值（3 个周期）使该窗口可忽略。默认 90000。0 = 默认。 |
