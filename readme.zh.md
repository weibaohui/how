...
**定时池健康巡检**（`healthcheckinterval`，仅客户端，默认 30s）。它**就是**客户端的死链检测器：每隔一个间隔，主动探测每条空闲隧道（发 ping、限时 10s 等 pong），**打印每条隧道的状态**（`pool health: idle=3 ok=3 ... |tunnel#1:ok(0s) ...`），关闭不应答的隧道并立刻补拨，把池子恢复到 `poolidlesize` 条**经过验证**的隧道——服务端随时都有可用连接，无需任何人重启客户端。它取代了旧的按连接被动回收（被动回收要累计 90s 完全静默才能**察觉**死链、且无法端到端验证）；`livenesstimeout` 仅作为上文所述的卡死兜底保留。若池子已完全清空且 connector 正在拨号退避中（服务端挂过又回来了），每轮巡检仍会按巡检节奏补拨一条"救援"隧道——恢复不再需要等退避结束，也无需重启客户端。
...
| `livenesstimeout` | 卡死兜底（毫秒）：写队列被塞满无法探活、且这段时间内未收到任何帧的空闲隧道判定为死并关闭。常规死链检测由主动健康巡检（探活 ping→pong，自带 10s 时限）承担，被动回收已被取代——因此任意正数配置均按原值生效（不再有 ping 周期下限）。默认 90000。0 = 默认。 |
